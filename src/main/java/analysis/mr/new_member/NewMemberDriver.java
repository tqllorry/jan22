 ...
 * @version：1.0
 */
public class NewMemberDriver implements Tool {
    private static Logger logger = Logger.getLogger(NewMemberDriver.class);
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "point_active");
        job.setJarByClass(MR_ActiveDriver.class);

        //map端
        job.setMapperClass(NewMemberMap.class);
    ...
    public static void main(String[] args) {
        ...
        try {
            ToolRunner.run(new Configuration(), new NewMemberDriver(), args);
        } catch (Exception e) {
            logger.error("执行异常", e);
        }
    }
